...
describe("runtime API proxy", () => {
  it("uses the runtime API origin and preserves the session response", async () => {
    ...
    const request = new NextRequest(
      "https://app.mensaly.online/api/v1/auth/login?source=test",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
  ...
  it("returns a stable safe error when the internal API is unavailable", async () => {
    ...
    const request = new NextRequest(
      "https://app.mensaly.online/api/v1/health/live",
    );
  ...
  it("forwards Mercado Pago payment signature headers", async () => {
    ...
    const fetchMock = vi.fn(
      async (_input: RequestInfo | URL, init?: RequestInit) => {
        expect((init?.headers as Headers).get("x-signature")).toBe(
          "ts=1,v1=signature",
        );
  ...
  it("preserves the authenticated cookie and redirect on the Mercado Pago OAuth callback", async () => {
    ...
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        expect(String(input)).toBe(
          "http://api:3001/api/v1/payment-integrations/mercadopago/callback?code=grant-1&state=state-1",
        );
        expect((init?.headers as Headers).get("cookie")).toBe(
          "mensaly_session=session-1",
        );
        return new Response(null, {
          status: 302,
          headers: {
            location:
              "https://app.mensaly.online/onboarding?step=payments&mercadopago=connected",
          },
        });
      },
    );
    ...
    const response = await GET(request, {
      params: Promise.resolve({
        path: ["payment-integrations", "mercadopago", "callback"],
      }),
    });
