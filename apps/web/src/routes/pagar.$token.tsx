  ...
  head: () => ({
    meta: [
      { title: "Pagamento da mensalidade — Mensaly" },
      {
        name: "description",
        content:
          "Checkout seguro para pagamento da mensalidade por Pix ou cartão.",
      },
    ],
  }),
...
function CheckoutPage() {
  ...
  useEffect(() => {
    let active = true;
    setError("");
    apiRequest<PublicCheckout>(
      `/public/mercadopago-checkout/${encodeURIComponent(token)}`,
    )
      .then((checkout) => {
        if (!active) return;
      ...
      .catch((reason: unknown) => {
        ...
        setError(
          reason instanceof Error
            ? reason.message
            : "Este checkout está indisponível.",
        );
      });
    return () => {
      active = false;
      ...
      <main className="flex min-h-screen items-center justify-center bg-muted/40 px-4">
        <div className="card-surface max-w-md p-8 text-center">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Peça um novo link para a escola.
          </p>
        </div>
      </main>
    );
  ...
  const unavailable = ["EXPIRED", "FAILED", "REFUNDED", "DISPUTED"].includes(
    details.status,
  );
      ...
      <div className="w-full max-w-xl space-y-6">
        <header className="flex flex-col items-center gap-3 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {brand?.segment || "Pagamento seguro"}
            </p>
          </div>
        ...
        <section className="card-surface overflow-hidden p-6 sm:p-7">
          {paid ? (
            <div className="flex flex-col items-center py-10 text-center">
              ...
              <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                O pagamento foi confirmado pelo provedor e registrado pela
                escola.
              </p>
            </div>
          ) : unavailable ? (
            ...
            <div className="flex flex-col items-center py-8 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                A tela será atualizada automaticamente após a confirmação do
                Mercado Pago.
              </p>
              {payment?.pix?.qrCodeBase64 ? (
                <img
                  ...
                  alt="QR Code Pix"
                  className="mt-5 size-56 rounded-lg border border-border"
                />
              ) : null}
              {payment?.pix?.qrCode ? (
                <Button
            ...
            <>
              <div
                ...
                role="group"
                aria-label="Escolha o meio de pagamento"
              >
                <Button
                  type="button"
                  variant="outline"
          ...
          <p className="mt-3 flex items-center justify-center gap-1.5 text-xs text-muted-foreground">
            <Lock className="size-3" /> Os dados sensíveis do cartão são
            tokenizados e não ficam armazenados na Mensaly.
          </p>
        </section>
        ...
        <aside className="card-surface h-fit p-6 sm:p-7">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between gap-4">
              ...
              <dd className="text-right text-foreground">
                {formatDateOnly(details.charge.dueDate)}
              </dd>
            </div>
            <div className="flex justify-between gap-4">
