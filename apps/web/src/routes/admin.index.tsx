...
export const Route = createFileRoute("/admin/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Saúde, uso, custos e crescimento da plataforma Mensaly.",
      },
    ],
  }),
...
function ChartLoading() {
  return (
    <div
      className="h-full w-full animate-pulse rounded-lg bg-muted/60 motion-reduce:animate-none"
...
function AdminOverviewPage() {
  ...
  const costRows = useMemo(
    () =>
      [...(analytics?.costs.organizations ?? [])].sort(
        (left, right) => right.estimatedTotalCents - left.estimatedTotalCents,
      ),
    [analytics],
  );
  const sentryStatus = analytics?.sentry.status;
  const totalOrganizations =
    analytics?.trends.at(-1)?.totalOrganizations ?? organizations.length;
  const totalActiveOrganizations =
    analytics?.costs.assumptions.activeOrganizations ?? activeOrgs;
    ...
    <AppShell variant="admin">
      ...
      {analyticsError ? (
        ...
        >
          ...
          Não foi possível carregar os indicadores avançados. Os dados básicos
      ...
      >
        <StatCard
          ...
          hint={`${totalActiveOrganizations} ativas`}
        />
        <StatCard
          label="Alunos ativos"
          ...
          value={
            !analytics
              ...
              : analytics.costs.configured
                ? formatCents(analytics.costs.totalEstimatedCents)
                : "Configurar"
          }
          hint={
            !analytics
              ...
              : analytics.costs.configured
                ? analytics.costs.referenceMonth
                : "Premissas ainda vazias"
          }
        />
        <StatCard
          ...
          value={
            sentryStatus === "ready"
              ? String(analytics?.sentry.totalErrors ?? 0)
              : "—"
          }
          hint={
            !analytics
              ? "Consultando integração"
              : sentryStatus === "not_configured"
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <article className="card-surface p-5">
          ...
          <p className="text-sm text-muted-foreground">
            Total acumulado e novos cadastros nos últimos 6 meses.
          </p>
          <div
            className="mt-5 h-72 w-full"
            aria-label="Gráfico de crescimento de empresas"
          >
            {!analytics ? (
              <ChartLoading />
            ) : (
        ...
        <article className="card-surface p-5">
          ...
          <p className="text-sm text-muted-foreground">
            Valores gerados e confirmados pelas escolas.
          </p>
          <div
            className="mt-5 h-72 w-full"
            aria-label="Gráfico de cobranças processadas"
          >
            {!analytics ? (
              <ChartLoading />
            ) : (
      ...
      {analytics ? (
        <table className="sr-only">
          <caption>
            Dados mensais dos gráficos de crescimento e cobranças
          </caption>
          <thead>
            <tr>
              <th>Mês</th>
          ...
          <tbody>
            {analytics.trends.map((item) => (
              <tr key={item.month}>
                <td>{item.month}</td>
                <td>{item.newOrganizations}</td>
                <td>{item.totalOrganizations}</td>
                <td>{formatCents(item.billedCents)}</td>
                <td>{formatCents(item.receivedCents)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <section className="grid gap-5 xl:grid-cols-3">
        <article className="card-surface p-5 xl:col-span-2">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-base font-semibold text-foreground">
                Erros monitorados pelo Sentry
              </h2>
              <p className="text-sm text-muted-foreground">
                Eventos aceitos por dia e principais problemas não resolvidos.
              </p>
            </div>
          ...
          <div
            ...
            aria-label="Gráfico de erros do Sentry"
          >
            {!analytics ? (
              <ChartLoading />
            ) : sentryStatus !== "ready" ? (
              <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-border px-6 text-center text-sm text-muted-foreground">
                {sentryStatus === "not_configured"
                  ? "Configure o acesso de leitura para visualizar os erros."
                  : "O Sentry não respondeu. Uma nova consulta será feita ao recarregar."}
              </div>
            ) : analytics.sentry.dailyErrors.length === 0 ? (
              <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-border px-6 text-center text-sm text-muted-foreground">
                Nenhum erro aceito pelo Sentry neste período.
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
                  data={analytics?.sentry.dailyErrors ?? []}
          ...
          {analytics?.sentry.dailyErrors.length ? (
            <p className="sr-only">
              Erros diários no período:{" "}
              {analytics.sentry.dailyErrors
                .map((item) => `${item.date}: ${item.errors}`)
                .join(", ")}
              .
            </p>
          ) : null}
                ...
                >
                  ...
                  {issue.permalink ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      aria-label={`Abrir ${issue.title} no Sentry`}
                    >
        ...
        <article className="card-surface p-5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Verificação direta dos serviços essenciais.
          </p>
          <ul className="mt-5 space-y-3">
            {platformHealth ? (
              Object.entries(platformHealth.dependencies).map(
                ([name, status]) => (
                  <li
                    key={name}
                  ...
                  >
                    ...
                    <StatusBadge
                      status={
                        status === "ready" ? "ACTIVE" : "FAILED_PERMANENT"
                      }
                    />
                  </li>
                ),
      ...
      <section className="card-surface overflow-hidden">
        <div className="border-b border-border px-5 py-4">
          ...
          <p className="text-sm text-muted-foreground">
            Rateio mensal da infraestrutura, e-mails e armazenamento. Não inclui
            taxas cobradas diretamente pelo Mercado Pago.
          </p>
        </div>
          ...
          <table className="w-full min-w-[840px] text-sm">
            <thead className="bg-muted/40 text-left text-xs font-medium uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-5 py-3">Rateio fixo</th>
            ...
            <tbody>
              {!analytics ? (
                <tr>
                  ...
                  >
                    Calculando os custos por empresa...
                  </td>
                </tr>
              ) : !analytics.costs.configured ? (
                <tr>
                  <td
                    ...
                    className="px-5 py-12 text-center text-muted-foreground"
                  >
                    Configure as premissas de custo no ambiente da API para
                    ativar esta estimativa.
                  </td>
                </tr>
              ) : (
                costRows.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-muted-foreground">
                      {formatBytes(row.storageBytes)} · {row.emailCount} e-mails
                    </td>
                    <td className="px-5 py-3 text-muted-foreground">
                      {formatCents(row.allocatedFixedCostCents)}
                    </td>
                    <td className="px-5 py-3 text-muted-foreground">
                      {formatCents(row.storageCostCents + row.emailCostCents)}
                    </td>
                    <td className="px-5 py-3 text-right font-semibold text-foreground">
                      {formatCents(row.estimatedTotalCents)}
                    </td>
                  </tr>
                ))
              )}
